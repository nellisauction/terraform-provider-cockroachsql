...
func TestCreateGrantRoleQuery(t *testing.T) {
	...
	for _, c := range cases {
		out := createGrantRoleQuery(schema.TestResourceDataRaw(t, resourceCockroachSQLGrantRole().Schema, c.resource))
		if out != c.expected {
...
func TestRevokeRoleQuery(t *testing.T) {
	...
	for _, c := range cases {
		out := createRevokeRoleQuery(schema.TestResourceDataRaw(t, resourceCockroachSQLGrantRole().Schema, c.resource))
		if out != expected {
...
func TestAccCockroachSQLGrantRole(t *testing.T) {
	skipIfNotAcc(t)
	...
	testAccCockroachSQLGrantRoleResources := fmt.Sprintf(`
	resource cockroachsql_role "grant" {
		name = "%s"
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"cockroachsql_grant_role.grant_role", "grant_role", grantedRoleName),
					resource.TestCheckResourceAttr(
						"cockroachsql_grant_role.grant_role", "with_admin_option", strconv.FormatBool(true)),
